...
#error not implemented
	...
	struct htlayout {
		...
		__device__ htlayout(equi200 *eq, u32 r) : hta(eq->hta), prevhashunits(0), dunits(0) {
...
__global__ void digitH(equi200 *eq) {
	uchar hash[HASHOUT];
	blake2b_state state;
	equi200::htlayout htl(eq, 0);
...
__global__ void digitO(equi200 *eq, const u32 r) {
	equi200::htlayout htl(eq, r);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void digitE(equi200 *eq, const u32 r) {
	equi200::htlayout htl(eq, r);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
...
#ifdef UNROLL
__global__ void digit_1(equi200 *eq) {
	equi200::htlayout htl(eq, 1);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit2(equi200 *eq) {
	equi200::htlayout htl(eq, 2);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit3(equi200 *eq) {
	equi200::htlayout htl(eq, 3);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit4(equi200 *eq) {
	equi200::htlayout htl(eq, 4);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit5(equi200 *eq) {
	equi200::htlayout htl(eq, 5);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit6(equi200 *eq) {
	equi200::htlayout htl(eq, 6);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit7(equi200 *eq) { 
	equi200::htlayout htl(eq, 7);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digit8(equi200 *eq) {
	equi200::htlayout htl(eq, 8);
	equi200::collisiondata cd;
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
__global__ void digitK(equi200 *eq) {
	equi200::collisiondata cd;
	equi200::htlayout htl(eq, WK);
	const u32 id = blockIdx.x * blockDim.x + threadIdx.x;
	for (u32 bucketid = id; bucketid < NBUCKETS; bucketid += eq->nthreads) {
		cd.clear();
...
eq_cuda_context200x9::eq_cuda_context200x9(int thrid, int devid, fn_validate validate, fn_cancel cancel)	
{
    threadsperblock = 64;
    device_id = devid;
    thread_id = thrid;
	...
	eq = new equi200(threadsperblock * totalblocks);
	...
	checkCudaErrors(cudaMalloc((void**)&device_eq, sizeof(equi200)));
}


eq_cuda_context200x9::~eq_cuda_context200x9()
{
	checkCudaErrors(cudaSetDevice(device_id));
	checkCudaErrors(cudaDeviceReset());
	free(sol_memory);
...
bool eq_cuda_context200x9::solve(unsigned char *pblock, unsigned char *header, unsigned int headerlen)
{
	checkCudaErrors(cudaSetDevice(device_id)); 

	eq->setheadernonce(header, headerlen-32, header+headerlen-32, 32);
	checkCudaErrors(cudaMemcpy(device_eq, eq, sizeof(equi200), cudaMemcpyHostToDevice));
	...
	checkCudaErrors(cudaMemcpy(eq, device_eq, sizeof(equi200), cudaMemcpyDeviceToHost));
