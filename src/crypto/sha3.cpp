...
void Sha3New::update(const std::string& input)
{
    this->update(input.c_str(), input.size());
}

void Sha3New::update(const std::vector<unsigned char>& input)
{
    this->update(input.data(), input.size());
}

void Sha3New::update(const void* inputBuffer, size_t length)
{
    sha3_Update(this, inputBuffer, length);
}

void Sha3New::finalize()
{
    sha3_Finalize(this);
}

std::string Sha3New::computeSha3_256(const std::string& input)
{
    this->init();
    this->update(input.c_str(), input.size());
    this->finalize();
...
std::string Sha3New::getResultString()
{
    std::string result;
    result.resize(32);
    for (int i = 0; i < 32; i ++) {
...
std::vector<unsigned char> Sha3New::getResultVector()
{
