                   ...
                   __global uint *rowCounters, __global uint *debug)
{
    ...
    while (input < input_end)
    {
...
#if FABCOIN_HASH_LEN == 50
	    h[6] = (blake_state[6] ^ v[6] ^ v[14]) & 0xffff;
#elif FABCOIN_HASH_LEN == 54
        h[6] = (blake_state[6] ^ v[6] ^ v[14]) & 0xffffffffffff;
#endif

	    // store the two Xi values in the hash table
#if FABCOIN_HASH_LEN == 50 || FABCOIN_HASH_LEN == 54
	    ...
	    dropped += ht_store(0, ht, input * 2 + 1,
		    ...
		    (h[6] >> 8), rowCounters);
#else
#error "unsupported FABCOIN_HASH_LEN"
#endif
	    input++;
    }
#ifdef ENABLE_DEBUG
...
part2:
    ...
    for (uint index = tlid; index < totalCollisions; index += get_local_size(0))
    {
        ...
        __global uchar *ptr = (__global uchar *)ht_src + collisionThreadId * NR_SLOTS * SLOT_LEN +
            xi_offset;
