...
###   fabcoincore.org and fabcoin.org and compares them.
...
if [ $? -ne 0 ]; then
   echo "fabcoin.org failed to provide signature file, but fabcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "fabcoin.org and fabcoincore.org signature files were not equal?"
