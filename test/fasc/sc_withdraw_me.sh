...
cat client_command.template|sed -e "s?FAB_ROOT?${FAB_ROOT}?"|sed -e "s?PORT_RPC?${port_rpc}?"|sed -e "s?CLIENTCOMMAND?sendtocontract?"|sed -e "s?PARAMLIST?${ADDR_SC_4} 3ccfd60b?" > tmp_send_contract.sr
echo "node 1 withdraw all coin from contract "
bash tmp_send_contract.sr
sleep 1
