...
echo
port_rpc=$(($RPC_PORT + 1 ))
cat client_command.template|sed -e "s?FAB_ROOT?${FAB_ROOT}?"|sed -e "s?PORT_RPC?${port_rpc}?"|sed -e "s?CLIENTCOMMAND?getcontractinfo?"|sed -e "s?PARAMLIST?${ADDR_SC_4}?" > tmp_get_account.sr
...
echo "node 2 get contact created at node 4 "
